...
export class API {
  ...
  // machineID 获取机器指纹（无鉴权接口），供持口令方派生 admin key。
  async machineID(): Promise<string> {
    const r = await this.req<{ machine_id: string }>("GET", "/api/v1/machine-id");
    return r.machine_id;
  }
  ...
  async messages(id: string, after = 0, limit = 100, before?: number): Promise<Message[]> {
    const q = before !== undefined
      ? `before=${before}&limit=${limit}`
      : `after=${after}&limit=${limit}`;
    const r = await this.req<{ messages: Message[] }>(
      "GET",
      `/api/v1/session/${id}/messages?${q}`,
    );
    return r.messages;
  }
