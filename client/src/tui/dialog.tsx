...
export function SessionSwitcher({
  ...
  useInput((input, key) => {
    if (isMouseSeq(input)) return; // 忽略鼠标事件，避免污染搜索框
  ...
  const previewLines = React.useMemo(() => {
    ...
    for (const m of preview) {
      ...
      // 每条最多 2 行非空内容
      const lines2 = (m.content?.text ?? "").split("\n").filter((l) => l.trim());
      const text = lines2.slice(0, 2).join(" ⏎ ") || "(empty)";
            ...
            return (
              <Box key={s.id} flexDirection="row" paddingLeft={1} paddingRight={1} backgroundColor={isSel ? theme.primary : undefined}>
                ...
                <Text color={isSel ? theme.background : theme.textMuted} wrap="truncate">
                  ...
                  {timeAgo(s.time_updated) ? ` · ${timeAgo(s.time_updated)}` : ""}
                  {!s.mine ? " · ro" : ""}
                </Text>
              </Box>
            );
