...
type Actor struct {
	...
	TimeUpdated int64  // 最近活跃时间（unix 秒），供列表按活跃度排序
	promptMode  string
...
func (m *Manager) resume(id string) (*Actor, bool) {
	...
	if err := m.db.QueryRow(`SELECT key_id, cwd, model, client_user, client_ip, COALESCE(time_updated,0) FROM session WHERE id = ?`, id).Scan(&keyID, &cwd, &model, &clientUser, &clientIP, &timeUpdated); err != nil {
		return nil, false
	}
	role := "user"
...
// SessionTimeUpdated 返回最近活跃时间（加锁读取，避免与 setStatus 写并发竞态）。
func (a *Actor) SessionTimeUpdated() int64 {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.TimeUpdated
}
...
func (a *Actor) appendMessage(role string, content any) {
	...
	a.broadcast(Event{Type: "message.created", Seq: seq, Data: map[string]any{"id": id, "role": role, "content": content, "created": now}})
}
...
func (a *Actor) setStatus(status string) {
	...
	a.TimeUpdated = now
	a.mu.Unlock()
	a.db.Exec(`UPDATE session SET status = ?, time_updated = ? WHERE id = ?`, status, now, a.ID)
